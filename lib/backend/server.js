...
app.get('/chuyenxe/:maCX', (req, res) => {
  ...
  db.query(sql, [maCX], (err, results) => {
    if (err) {
      console.error('❌ Lỗi truy vấn chuyến xe theo MaCX:', err);
...
app.get('/api/ve', (req, res) => {
  ...
  db.query(getMaKHQuery, [username], (err, tkRows) => {
    ...
    if (filter === 'today') {
      ...
      query += ' AND CHUYENXE.ThoiGianDi BETWEEN DATE_SUB(CURDATE(), INTERVAL 6 DAY) AND CURDATE()';
    } else if (filter === '30days') {
      query += ' AND CHUYENXE.ThoiGianDi BETWEEN DATE_SUB(CURDATE(), INTERVAL 29 DAY) AND CURDATE()';
    }
...
app.post('/add-taikhoankh', (req, res) => {
  const { MaTK, TenDangNhapKH, Password, MaKH } = req.body;

  if (!MaTK || !TenDangNhapKH || !Password || !MaKH) {
    return res.status(400).json({ success: false, message: 'Thiếu dữ liệu đầu vào' });
  }

  const sql = 'INSERT INTO TAIKHOANKH (MaTK, TenDangNhapKH, Password, MaKH) VALUES (?, ?, ?, ?)';
  db.query(sql, [MaTK, TenDangNhapKH, Password, MaKH], (err, result) => {
    if (err) {
      console.error('Lỗi khi thêm tài khoản khách hàng:', err);
      return res.status(500).json({ success: false, message: 'Không thể thêm tài khoản khách hàng' });
    }

    res.json({ success: true, message: 'Thêm tài khoản khách hàng thành công' });
  });
});

// Đọc tất cả tài khoản khách hàng
app.get('/taikhoankh', (req, res) => {
  const sql = 'SELECT * FROM TAIKHOANKH';
  db.query(sql, (err, results) => {
    if (err) {
      console.error('Lỗi khi lấy danh sách tài khoản KH:', err);
      return res.status(500).json({ success: false, message: 'Không thể lấy dữ liệu' });
    }
...
app.put('/taikhoankh/:maTK', (req, res) => {
  ...
  const sql = 'UPDATE TAIKHOANKH SET TenDangNhapKH = ?, Password = ?, MaKH = ? WHERE MaTK = ?';
  const params = [TenDangNhapKH, Password, MaKH, maTK];
...
app.delete('/taikhoankh/:maTK', (req, res) => {
  const maTK = req.params.maTK;

  const sql = 'DELETE FROM TAIKHOANKH WHERE MaTK = ?';
  db.query(sql, [maTK], (err, result) => {
    if (err) {
...
app.post('/add-khachhang', (req, res) => {
  const { MaKH, HoVaTen, NgaySinh, DiaChi, Email, SDT, URLHinhAnh } = req.body;

  if (!MaKH || !HoVaTen || !NgaySinh || !DiaChi || !Email || !SDT || !URLHinhAnh) {
    return res.status(400).json({ success: false, message: 'Thiếu dữ liệu khách hàng' });
  }

  const sql = 'INSERT INTO KHACHHANG (MaKH, HoVaTen, NgaySinh, DiaChi, Email, SDT, URLHinhAnh) VALUES (?, ?, ?, ?, ?, ?, ?)';
  db.query(sql, [MaKH, HoVaTen, NgaySinh, DiaChi, Email, SDT, URLHinhAnh], (err, result) => {
    if (err) {
...
app.get('/khachhang', (req, res) => {
  ...
  db.query(sql, (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ success: false, message: 'Không thể lấy danh sách khách hàng' });
    }
...
app.get('/khachhang/:MaKH', (req, res) => {
  ...
  db.query(sql, [MaKH], (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ success: false, message: 'Không thể lấy thông tin khách hàng' });
    }
...
app.put('/khachhang/:MaKH', (req, res) => {
  const { MaKH } = req.params;
  const { HoVaTen, NgaySinh, DiaChi, Email, SDT, URLHinhAnh } = req.body;

  const sql = `
    UPDATE KHACHHANG 
    SET HoVaTen = ?, NgaySinh = ?, DiaChi = ?, Email = ?, SDT = ?, URLHinhAnh = ?
    WHERE MaKH = ?`;

  db.query(sql, [HoVaTen, NgaySinh, DiaChi, Email, SDT, URLHinhAnh, MaKH], (err, result) => {
    if (err) {
...
app.delete('/khachhang/:MaKH', (req, res) => {
  ...
  db.query(sql, [MaKH], (err, result) => {
    if (err) {
      ...
      return res.status(500).json({ success: false, message: 'Không thể xóa khách hàng' });
    }
...
app.post('/nhanvien', (req, res) => {
  const {
    MaNV, HoVaTen, NgaySinh, DiaChi, Email, SDT,
    URLHinhAnh, NgayVaoLam, ChucVu, PhongBan
  } = req.body;

  const sql = `
    INSERT INTO NHANVIEN 
    (MaNV, HoVaTen, NgaySinh, DiaChi, Email, SDT, URLHinhAnh, NgayVaoLam, ChucVu, PhongBan) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
  ...
  db.query(sql, values, (err, result) => {
    if (err) {
      ...
      return res.status(500).json({ success: false, message: 'Không thể thêm nhân viên' });
    }
...
app.put('/nhanvien/:MaNV', (req, res) => {
  const { MaNV } = req.params;
  const {
    HoVaTen, NgaySinh, DiaChi, Email, SDT,
    URLHinhAnh, NgayVaoLam, ChucVu, PhongBan
  } = req.body;

  const sql = `
    UPDATE NHANVIEN 
    SET HoVaTen = ?, NgaySinh = ?, DiaChi = ?, Email = ?, SDT = ?, 
        URLHinhAnh = ?, NgayVaoLam = ?, ChucVu = ?, PhongBan = ?
    WHERE MaNV = ?`;
  ...
  db.query(sql, values, (err, result) => {
    if (err) {
      ...
      return res.status(500).json({ success: false, message: 'Không thể cập nhật nhân viên' });
    }
...
// Delete employee
app.delete('/nhanvien/:MaNV', (req, res) => {
  const { MaNV } = req.params;
...
app.post('/taikhoannv', (req, res) => {
  const { MaTK, TenDangNhapNV, Password, MaNV } = req.body;

  if (!MaTK || !TenDangNhapNV || !Password || !MaNV) {
    return res.status(400).json({ success: false, message: 'Thiếu dữ liệu' });
  }

  const sql = 'INSERT INTO TAIKHOANNV (MaTK, TenDangNhapNV, Password, MaNV) VALUES (?, ?, ?, ?)';
  db.query(sql, [MaTK, TenDangNhapNV, Password, MaNV], (err, result) => {
    if (err) {
      console.error('❌ Lỗi khi thêm tài khoản NV:', err);
...
// 📖 Read all employee accounts
app.get('/taikhoannv', (req, res) => {
  const sql = 'SELECT * FROM TAIKHOANNV';
  db.query(sql, (err, results) => {
    if (err) {
      console.error('❌ Lỗi khi lấy danh sách tài khoản NV:', err);
      return res.status(500).json({ success: false, message: 'Không thể lấy dữ liệu' });
    }
    res.json(results);
  });
...
// 🔍 Read one employee account by MaTK
app.get('/taikhoannv/:MaTK', (req, res) => {
  const { MaTK } = req.params;
  const sql = 'SELECT * FROM TAIKHOANNV WHERE MaTK = ?';
  db.query(sql, [MaTK], (err, results) => {
    if (err) {
      console.error('❌ Lỗi khi lấy tài khoản NV:', err);
      return res.status(500).json({ success: false, message: 'Không thể lấy dữ liệu' });
    }
...
// ✏️ Update employee account
app.put('/taikhoannv/:MaTK', (req, res) => {
  const { MaTK } = req.params;
  const { TenDangNhapNV, Password, MaNV } = req.body;

  if (!TenDangNhapNV || !Password || !MaNV) {
    return res.status(400).json({ success: false, message: 'Thiếu dữ liệu cập nhật' });
  }

  const sql = 'UPDATE TAIKHOANNV SET TenDangNhapNV = ?, Password = ?, MaNV = ? WHERE MaTK = ?';
  db.query(sql, [TenDangNhapNV, Password, MaNV, MaTK], (err, result) => {
    if (err) {
      console.error('❌ Lỗi khi cập nhật tài khoản NV:', err);
...
app.delete('/taikhoannv/:MaTK', (req, res) => {
  const { MaTK } = req.params;

  const sql = 'DELETE FROM TAIKHOANNV WHERE MaTK = ?';
  db.query(sql, [MaTK], (err, result) => {
    if (err) {
      console.error('❌ Lỗi khi xóa tài khoản NV:', err);
...
app.get('/lochuyenxe', (req, res) => {
  ...
  const sql = `
    SELECT * FROM chuyenxe
    WHERE DiemDi = ? AND DiemDen = ? AND DATE(ThoiGianDi) = ?
  `;

  db.query(sql, [diemDi, diemDen, ngayDi], (err, results) => {
    if (err) {
      console.error('❌ Lỗi truy vấn MySQL:', err);
      return res.status(500).json({ success: false, message: 'Lỗi server' });
    }

    res.json(results); // Trả về danh sách đã lọc đúng yêu cầu
  });
});

//API lọc khách hàng theo mong muốn
app.get('/lockhachhang', (req, res) => {
  const { maKH, hoVaTen, ngaySinh, diaChi, email, sdt, urlHinhAnh } = req.query;
  ...
  if (urlHinhAnh) {
    sql += " AND URLHinhAnh LIKE ?";
    params.push(`%${urlHinhAnh}%`);
  }
...
//API lọc nhân viên
app.get('/locnhanvien', (req, res) => {
  const { maNV, hoVaTen, ngaySinh, diaChi, email, sdt, urlHinhAnh, ngayVaoLam, chucVu, phongBan } = req.query;
  ...
  db.query(sql, params, (err, results) => {
    if (err) {
      ...
      return res.status(500).json({ success: false, message: 'Lỗi server khi lọc nhân viên' });
    }
...
// API lọc tài khoản khách hàng
app.get('/loctaikhoankh', (req, res) => {
  const { maTK, tenDangNhapKH, password, maKH } = req.query;

  let sql = "SELECT * FROM TAIKHOANKH WHERE 1=1";
  const params = [];
  ...
  if (tenDangNhapKH) {
    sql += " AND TenDangNhapKH LIKE ?";
    params.push(`%${tenDangNhapKH}%`);
  }
  ...
  db.query(sql, params, (err, results) => {
    if (err) {
      console.error('❌ Lỗi truy vấn tài khoản KH:', err);
      return res.status(500).json({ success: false, message: 'Lỗi server khi lọc tài khoản khách hàng' });
    }
...
// API lọc tài khoản nhân viên
app.get('/loctaikhoannv', (req, res) => {
  const { maTK, tenDangNhapNV, password, maNV } = req.query;

  let sql = "SELECT * FROM TAIKHOANNV WHERE 1=1";
  const params = [];
  ...
  if (tenDangNhapNV) {
    sql += " AND TenDangNhapNV LIKE ?";
    params.push(`%${tenDangNhapNV}%`);
  }
  ...
  db.query(sql, params, (err, results) => {
    if (err) {
      console.error('❌ Lỗi truy vấn tài khoản NV:', err);
      return res.status(500).json({ success: false, message: 'Lỗi server khi lọc tài khoản nhân viên' });
    }
...
app.get('/thongke/doanhthu', async (req, res) => {
  ...
  try {
    const sql = `
      ...
      WHERE v.TrangThai = 'Đã thanh toán'
        AND DATE(cx.ThoiGianDi) BETWEEN STR_TO_DATE(?, '%d-%m-%Y') 
...
// API lấy danh sách vé theo khoảng ngày
app.get('/thongke/danhsachve', async (req, res) => {
  const { startDate, endDate } = req.query;
  ...
  try {
    const sql = `
      SELECT v.MaVe, cx.MaCX, cx.ThoiGianDi AS NgayDi, v.ViTriGheNgoi, v.GiaVe
      FROM VE v
      JOIN CHUYENXE cx ON v.MaCX = cx.MaCX
      WHERE v.TrangThai = 'Đã thanh toán'
        AND DATE(cx.ThoiGianDi) BETWEEN STR_TO_DATE(?, '%d-%m-%Y') AND STR_TO_DATE(?, '%d-%m-%Y')
      ORDER BY cx.ThoiGianDi ASC
    `;

    // Chuyển hàm callback sang Promise để dùng await
