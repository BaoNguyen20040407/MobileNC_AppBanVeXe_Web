...
class _TripListState extends State<TripList> {
  ...
  String selectedColumn = 'MaCX'; // Mặc định tìm theo Mã CX
  ...
  Future<void> fetchTrips() async {
    ...
    if (response.statusCode == 200) {
      ...
      print('Lỗi khi lấy dữ liệu chuyến xe');
    }
  }
      ...
      pw.Page(
        build: (pw.Context context) {
          return pw.Table.fromTextArray(
            headers: [
              'Mã CX', 'Biển số', 'Thời gian đi', 'Thời gian về',
              'Điểm đi', 'Điểm đến', 'Loại hình', 'Giá vé', 'Số chỗ'
            ],
            data: data.map((trip) {
              return [
                trip['MaCX'] ?? '',
                trip['BienSoXe'] ?? '',
                trip['ThoiGianDi']?.toString().substring(0, 16) ?? '',
                trip['ThoiGianVe']?.toString().substring(0, 16) ?? '',
                trip['DiemDi'] ?? '',
                trip['DiemDen'] ?? '',
                trip['LoaiHinhChuyenDi'] ?? '',
                trip['GiaVe'].toString(),
                trip['SoChoNgoi'].toString(),
              ];
            }).toList(),
  ...
  Widget build(BuildContext context) {
    List<dynamic> filteredList = tripList.where((trip) {
      final value = (trip[selectedColumn] ?? '').toString().toLowerCase();
                        ...
                        Column(
                          children: [
                            ...
                            SingleChildScrollView(
                              ...
                              child: Row(
                                children: [
                                  ChoiceChipSelector(label: 'Mã CX', value: 'MaCX', selectedValue: selectedColumn, onSelected: (val) => setState(() => selectedColumn = val)),
                                  ...
                                  const SizedBox(width: 8),
                                  ChoiceChipSelector(label: 'Điểm đến', value: 'DiemDen', selectedValue: selectedColumn, onSelected: (val) => setState(() => selectedColumn = val)),
                                ],
                              ),
                            ),
                          ],
                      ...
                      Container(
                        ...
                        child: SingleChildScrollView(
                          ...
                          child: DataTable(
                            ...
                            dataRowMaxHeight: 48,
                            columns: const [
                              DataColumn(label: SizedBox(width: 80, child: Text('Mã CX', style: TextStyle(fontWeight: FontWeight.bold, fontFamily: 'Inter')))),
                              DataColumn(label: SizedBox(width: 120, child: Text('Biển số', style: TextStyle(fontWeight: FontWeight.bold, fontFamily: 'Inter')))),
                              DataColumn(label: SizedBox(width: 160, child: Text('Điểm đến', style: TextStyle(fontWeight: FontWeight.bold, fontFamily: 'Inter')))),
                            ],
                            rows: filteredList.map((trip) {
                              return DataRow(
                                cells: [
                                  DataCell(SizedBox(
                                    width: 80,
                                    child: InkWell(
                                      onTap: () {
                                        Navigator.push(
                                          context,
                                          MaterialPageRoute(builder: (context) => EditTrip(trip: trip)),
                                        );
                                      },
                                      child: Text(
                                        trip['MaCX'] ?? '',
                                  ...
                                  DataCell(SizedBox(width: 120, child: Text(trip['BienSoXe'] ?? '', overflow: TextOverflow.ellipsis, style: const TextStyle(fontFamily: 'Inter')))),
                                  //DataCell(SizedBox(width: 180, child: Text(trip['ThoiGianDi']?.toString().substring(0, 16) ?? '', style: const TextStyle(fontFamily: 'Inter')))),
                                  //DataCell(SizedBox(width: 180, child: Text(trip['ThoiGianVe']?.toString().substring(0, 16) ?? '', style: const TextStyle(fontFamily: 'Inter')))),
                                  DataCell(SizedBox(width: 160, child: Text(trip['DiemDen'] ?? '', style: const TextStyle(fontFamily: 'Inter')))),
                                ],
                              );
                            }).toList(),
                          ),
